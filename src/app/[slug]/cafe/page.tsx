...
export default function CafePage() {
  ...
  const loadItems = useCallback(async () => {
    const res = await fetch(`/api/${slug}/kds/cafe`);
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col">
      <header className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        <div>
          <div className="font-bold text-amber-400 text-lg">ກາເຟ — KDS</div>
          <div className="text-xs text-gray-400">{user?.name} · ລໍຖ້າ: <span className="text-yellow-400 font-bold">{pendingCount}</span></div>
        </div>
